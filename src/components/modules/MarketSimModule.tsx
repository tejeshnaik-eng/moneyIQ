...
const Tooltip = ({ children, content }: { children: React.ReactNode, content: string }) => {
  return (
    <div className="relative inline-flex flex-col items-center group cursor-help">
      ...
      <div className="absolute bottom-full mb-2 hidden group-hover:block w-max max-w-[250px] bg-[#1A1A1A] text-white text-xs p-2 rounded-2xl shadow-lg 6E7C75]/20 z-[100] whitespace-normal text-left">
        {content}
      </div>
    </div>
  );
...
function FinancialChart({ data, width, height, type }: { data: OHLCV[]; width: number; height: number; type: 'line' | 'candlestick' }) {
  ...
  if (!data || data.length === 0) return <div className="flex h-full items-center justify-center text-[#A1A1AA]">No chart data available</div>;
  ...
  return (
    <div className="relative w-full h-full select-none" onMouseLeave={() => setHoveredIndex(null)}>
      ...
      {hoveredIndex !== null && data[hoveredIndex] && (
        <div className="absolute top-2 right-2 bg-[#252525] text-[#A1A1AA] text-xs p-2 rounded-2xl shadow flex space-x-3 font-['Hedvig_Letters_Sans'] 20EFA0]/20 z-10">
          <span className="text-white">{new Date(data[hoveredIndex].time).toLocaleDateString()}</span>
          <span>O: <span className="text-white">{data[hoveredIndex].open.toFixed(2)}</span></span>
          <span>H: <span className="text-white">{data[hoveredIndex].high.toFixed(2)}</span></span>
          <span>L: <span className="text-white">{data[hoveredIndex].low.toFixed(2)}</span></span>
          <span>C: <span className="text-white">{data[hoveredIndex].close.toFixed(2)}</span></span>
          <span>V: <span className="text-white">{(data[hoveredIndex].volume / 1000).toFixed(1)}k</span></span>
        </div>
      )}
    </div>
  );
...
export const MarketSimModule: React.FC = () => {
  ...
  const renderTabContent = () => {
    if (loading) return <div className="p-4 text-[#A1A1AA]">Loading data...</div>;
    if (!stockData || !currentDayData) return <div className="p-4 text-[#A1A1AA]">No data available</div>;
        ...
        <div className="p-6 space-y-6 font-['Outfit'] pb-20">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-[#252525] rounded-2xl-[24px] p-5 6E7C75]/20 space-y-6">
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <div><p className="text-[#8A8F98]">Today's Low</p><p className="text-white font-medium">₹{todayLow.toFixed(2)}</p></div>
                    <div className="text-right"><p className="text-[#8A8F98]">Today's High</p><p className="text-white font-medium">₹{todayHigh.toFixed(2)}</p></div>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <div><p className="text-[#8A8F98]">52W Low</p><p className="text-white font-medium">₹{w52Low.toFixed(2)}</p></div>
                    <div className="text-right"><p className="text-[#8A8F98]">52W High</p><p className="text-white font-medium">₹{w52High.toFixed(2)}</p></div>
                  </div>
                  <div className="relative h-2 bg-[#2A2A2A] rounded-2xl-full">
                    <div className="absolute top-0 bottom-0 left-0 bg-[#20EFA0] rounded-2xl-full" style={{width: '100%'}}></div>
                ...
                <div className="pt-4 6E7C75]/20 grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div><p className="text-[#8A8F98] text-xs">Open</p><p className="text-white text-sm font-medium">{currentDayData.open.toFixed(2)}</p></div>
                  <div><p className="text-[#8A8F98] text-xs">Prev. Close</p><p className="text-white text-sm font-medium">{prevClose.toFixed(2)}</p></div>
                  <div><p className="text-[#8A8F98] text-xs">Volume</p><p className="text-white text-sm font-medium">{(currentDayData.volume/1000).toFixed(2)}k</p></div>
                  <div><p className="text-[#8A8F98] text-xs">Lower Circuit</p><p className="text-white text-sm font-medium">{lowerCircuit.toFixed(2)}</p></div>
                  <div><p className="text-[#8A8F98] text-xs">Upper Circuit</p><p className="text-white text-sm font-medium">{upperCircuit.toFixed(2)}</p></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#252525] p-4 rounded-2xl-[24px] 6E7C75]/20 flex justify-between">
                  <Tooltip content="Total market value of a company's outstanding shares"><span className="text-[#8A8F98] text-sm">Market Cap</span></Tooltip>
                  <span className="text-white font-medium">{stockData.marketCap ? `₹${(stockData.marketCap / 10000000).toFixed(2)}Cr` : 'N/A'}</span>
                </div>
                <div className="bg-[#252525] p-4 rounded-2xl-[24px] 6E7C75]/20 flex justify-between">
                  <Tooltip content="Price-to-Earnings ratio"><span className="text-[#8A8F98] text-sm">P/E Ratio</span></Tooltip>
                  <span className="text-white font-medium">{stockData.peRatio?.toFixed(2) || 'N/A'}</span>
                </div>
                <div className="bg-[#252525] p-4 rounded-2xl-[24px] 6E7C75]/20 flex justify-between">
                  <Tooltip content="Return on Equity"><span className="text-[#8A8F98] text-sm">ROE</span></Tooltip>
                  <span className="text-white font-medium">{'N/A'}</span>
                </div>
                <div className="bg-[#252525] p-4 rounded-2xl-[24px] 6E7C75]/20 flex justify-between">
                  <Tooltip content="Annual dividend payment relative to stock price"><span className="text-[#8A8F98] text-sm">Dividend Yield</span></Tooltip>
                  <span className="text-white font-medium">{'N/A'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#252525] rounded-2xl-[24px] p-5 6E7C75]/20">
                <div className="flex justify-between text-sm mb-2">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="grid grid-cols-2 text-[#8A8F98] mb-2"><span className="text-left">Bid</span><span className="text-right">Qty</span></div>
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className="grid grid-cols-2 text-white mb-2"><span className="text-left">0.00</span><span className="text-right">0</span></div>
                    ))}
                  ...
                  <div>
                    <div className="grid grid-cols-2 text-[#8A8F98] mb-2"><span className="text-left">Ask</span><span className="text-right">Qty</span></div>
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className="grid grid-cols-2 text-white mb-2"><span className="text-left">0.00</span><span className="text-right">0</span></div>
                    ))}
    ...
    if (activeTab === 'Technicals') {
      ...
      return (
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 font-['Outfit']">
          <div className="bg-[#252525] p-5 rounded-2xl-[24px] 6E7C75]/20 flex items-center justify-between">
            <div>
              <p className="text-[#8A8F98] text-sm mb-1">
                <Tooltip content="Relative Strength Index - momentum oscillator">RSI (14)</Tooltip>
              </p>
              <p className="text-white text-3xl font-medium">{currentRSI?.toFixed(2) || 'N/A'}</p>
            </div>
          ...
          <div className="bg-[#252525] p-5 rounded-2xl-[24px] 6E7C75]/20">
            <p className="text-[#8A8F98] text-sm mb-1">
              <Tooltip content="Simple Moving Average">SMA (20)</Tooltip>
            </p>
            <p className="text-white text-3xl font-medium">{currentSMA?.toFixed(2) || 'N/A'}</p>
    ...
    if (activeTab === 'F&O') {
      return (
        <div className="p-6 font-['Outfit'] space-y-6">
          <div className="bg-[#252525] rounded-2xl-[24px] 6E7C75]/20 overflow-hidden">
            <div className="p-4 bg-[#1C1C1C] 6E7C75]/20 flex justify-between items-center">
              <h3 className="text-white font-medium">Option Chain (Example Data)</h3>
              <span className="text-xs bg-[#2A2A2A] text-[#A1A1AA] px-2 py-1 rounded-2xl">Exp: 28 Sep 2026</span>
            </div>
            <div className="p-4 grid grid-cols-3 text-sm text-center 6E7C75]/20">
              <div className="text-[#A1A1AA]">CALLS (LTP)</div>
              <div className="text-white font-medium">STRIKE</div>
              <div className="text-[#A1A1AA]">PUTS (LTP)</div>
            </div>
            {[1400, 1450, 1500, 1550, 1600].map((strike, i) => (
              <div key={strike} className={`grid grid-cols-3 text-sm text-center p-3 ${i % 2 === 0 ? 'bg-[#1C1C1C]/50' : ''}`}>
                <div className="text-white">{(Math.random() * 50 + 10).toFixed(2)}</div>
    ...
    if (activeTab === 'Events') {
      return (
        <div className="p-6 font-['Outfit'] space-y-6">
          <div className="bg-[#252525] rounded-2xl-[24px] 6E7C75]/20 p-5">
            <h3 className="text-white font-medium mb-4">Company Events (Example Data)</h3>
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-[#6E7C75]/20 before:to-transparent">
              {[
                ...
                <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-5 h-5 rounded-2xl-full 20EFA0] bg-[#252525] text-[#20EFA0] shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2"></div>
                  <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] p-4 rounded-2xl-[24px] bg-[#1C1C1C] 6E7C75]/20 shadow">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-[#A1A1AA]">{ev.date}</span>
                      <span className="text-[10px] bg-[#2A2A2A] text-white px-2 py-0.5 rounded-2xl">{ev.type}</span>
                    </div>
                    <h4 className="text-white font-medium text-sm">{ev.title}</h4>
                  </div>
                </div>
              ))}
            </div>
    ...
    return (
      <div className="h-40 flex flex-col items-center justify-center text-[#A1A1AA] font-['Outfit']">
  ...
  return (
    <div className="flex flex-col h-screen bg-transparent text-white font-['Outfit'] overflow-hidden">
      
      {/* Header */}
      <header className="flex-none bg-[#1C1C1C] ] p-4 flex justify-between items-center z-10">
        <div className="flex items-center space-x-6">
          <div className="relative group">
            <div className="flex items-center bg-[#252525] rounded-2xl-[20px] px-3 py-1.5 6E7C75]/20">
              <Search className="w-4 h-4 text-[#A1A1AA] mr-2" />
              <input 
                className="bg-transparent text-white outline-none placeholder-[#6E7C75] w-48 text-sm"
            ...
            {searchQuery && searchResults.length > 0 && (
              <div className="absolute top-full left-0 w-full mt-1 bg-[#252525] 6E7C75]/20 rounded-2xl-[20px] shadow-xl overflow-hidden z-20">
                {searchResults.map(res => (
                  <button 
                    key={res.symbol}
                    className="w-full text-left px-4 py-2 hover:bg-[#1C1C1C] flex flex-col transition-colors"
                    onClick={() => {
                      setSelectedSymbol(res.symbol);
                      setSearchQuery('');
                    }}
                  >
                    <span className="font-medium text-white">{res.symbol}</span>
                    <span className="text-xs text-[#8A8F98]">{res.name}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          {!loading && stockData && currentDayData && (
            <div className="flex items-center space-x-4 ] pl-6">
              <div>
                <h1 className="text-xl font-bold font-['Hedvig_Letters_Sans']">{stockData.companyName} <span className="text-sm font-normal text-[#A1A1AA]">({stockData.symbol})</span></h1>
        ...
        <div className="flex bg-[#252525] rounded-2xl-full p-1 6E7C75]/20">
          <button 
            className={`px-4 py-1.5 rounded-2xl-full text-sm font-medium transition-colors ${mode === 'RESEARCH' ? 'bg-[#2A2A2A] text-white' : 'text-[#A1A1AA] hover:text-white'}`}
            onClick={() => setMode('RESEARCH')}
          >
            Research
          </button>
          <button 
            className={`px-4 py-1.5 rounded-2xl-full text-sm font-medium transition-colors ${mode === 'SIMULATE' ? 'bg-[#2A2A2A] text-white' : 'text-[#A1A1AA] hover:text-white'}`}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-transparent">
          ...
          <div className="flex-1 min-h-[300px] ] p-6 flex flex-col" ref={containerRef}>
            {loading ? (
              <div className="h-full flex items-center justify-center text-[#A1A1AA]">Loading chart...</div>
            ) : (
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-end space-x-2 mb-4">
                      <span className="text-3xl font-bold text-white">₹{currentData?.close.toFixed(2) || '0.00'}</span>
                      <span className={`text-sm font-medium flex items-center pb-1 ${changeColor}`}>
                        ...
                        <span className="text-[#A1A1AA] ml-2 font-normal">{timeframe}</span>
                      </span>
                    </div>
                  );
                })()}
                ...
                <div className="h-10 flex items-center justify-between ] pt-4 mt-auto">
                  <div className="flex items-center space-x-1">
                    {['1D', '1W', '1M', '3M', '6M', '1Y', '3Y', '5Y', 'All'].map(tf => (
                      <button 
                        ...
                        className={`px-3 py-1.5 text-xs font-medium rounded-2xl-full transition-colors ${timeframe === tf ? 'bg-[#2A2A2A] text-white' : 'bg-transparent text-[#A1A1AA] hover:bg-[#1A1A1A] hover:text-white'}`}
                      >
                        {tf}
                      </button>
                    ))}
                  ...
                  <div className="flex items-center space-x-2">
                    <button 
                      ...
                      className={`p-1.5 rounded-2xl-full transition-colors ${chartType === 'candlestick' ? 'bg-[#2A2A2A] text-white' : 'bg-transparent text-[#A1A1AA] hover:bg-[#1A1A1A] hover:text-white'}`}
                      title="Toggle Chart Type"
                    >
                      {chartType === 'line' ? <BarChart2 className="w-4 h-4" /> : <LineChart className="w-4 h-4" />}
                    </button>
                    <div className="w-px h-4 bg-[#252525]"></div>
                    <button className="flex items-center px-3 py-1.5 text-xs font-medium rounded-2xl-full text-[#A1A1AA] bg-transparent hover:bg-[#1A1A1A] hover:text-white transition-colors">
          ...
          {/* Tabs Section */}
          <div className="h-1/3 min-h-[250px] bg-[#1C1C1C] flex flex-col">
            <div className="flex space-x-6 px-6 ]">
              {(['Overview', 'Technicals', 'News', 'Events', 'F&O'] as Tab[]).map(tab => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`py-3 px-2 text-sm font-medium transition-colors ${ activeTab === tab ? 'F2F7F4] text-white' : ' text-[#A1A1AA] hover:text-white' }`}
                >
                  {tab}
                </button>
              ))}
        ...
        {mode === 'SIMULATE' && (
          <div className="w-80 bg-[#1C1C1C] ] flex flex-col">
            <div className="p-4 ]">
              ...
              <div className="bg-[#252525] p-4 rounded-2xl-[24px] space-y-3 mb-6">
                <div className="flex justify-between text-sm">
                  <span className="text-[#8A8F98]">Portfolio Value</span>
                  <span className="text-white font-medium font-['Hedvig_Letters_Sans']">₹{simPortfolioValue.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#8A8F98]">Available Cash</span>
                  <span className="text-white font-medium font-['Hedvig_Letters_Sans']">₹{simCash.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#8A8F98]">Holdings</span>
                  <span className="text-white font-medium font-['Hedvig_Letters_Sans']">{simShares} shares</span>
                </div>
                <div className="pt-2 6E7C75]/20 flex justify-between text-sm">
                  <span className="text-[#8A8F98]">Return</span>
              ...
              <div className="mb-6 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[#A1A1AA]">Current Date</span>
                  <span className="text-white font-medium">{currentDayData ? new Date(currentDayData.time).toLocaleDateString() : '--'}</span>
                </div>
                <button 
                  onClick={handleStepForward}
                  disabled={!stockData || simIndex >= stockData.history.length - 1}
                  className="w-full py-2.5 bg-[#2A2A2A] hover:bg-[#333333] text-white rounded-2xl-full flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[#A1A1AA]">Trade Quantity</span>
                  <input 
                    type="number" 
                    value={tradeQuantity}
                    onChange={(e) => setTradeQuantity(Math.max(1, parseInt(e.target.value) || 1))}
                    className="w-20 bg-[#252525] 6E7C75]/30 rounded-2xl px-2 py-1 text-right outline-none text-white"
                    min="1"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className="py-2 bg-[#2A2A2A] hover:bg-[#333333] text-white rounded-2xl-full font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Buy
                  </button>
                  <button 
