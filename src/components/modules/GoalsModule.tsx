...
export const GoalsModule: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'On Track':
        return <span className="bg-[var(--primary)]/10 text-[var(--primary-dim)] text-[11px] font-heading font-bold px-2 py-0.5 rounded">On Track</span>;
      case 'Attention Needed':
        return <span className="bg-[var(--app-surface-alt)] text-[#e59840] text-[11px] font-heading font-bold px-2 py-0.5 rounded">Review Needed</span>;
      default:
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      default:
        return <Plane className="w-5 h-5 text-[var(--primary-dim)]" />;
    }
  };
  ...
  return (
    <div className="space-y-8 pb-12">
      {/* Page Header */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-[var(--app-border)] pb-4">
        <div>
          <span className="text-xs font-mono text-[var(--primary-dim)] font-semibold uppercase tracking-wider">
            Capital Allocation Engine
          </span>
          <h3 className="text-xl font-heading font-extrabold text-[var(--app-text)] mt-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {goals.map((goal) => {
            ...
            return (
              <div
                key={goal.id}
                className="bg-[var(--app-surface)] rounded-xl border border-[var(--app-border)] p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow"
              >
                <div>
                  ...
                  <h4 className="font-heading font-bold text-lg text-[var(--app-text)] mb-0.5">
                    {goal.title}
                  </h4>
                  <p className="text-xs text-[var(--app-text-muted)] mb-6">
                    {goal.category === 'Security' ? '6 months runway liquidity cushion' : 'Target milestone corpus'}
                  </p>

                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between items-baseline">
                      <span className="text-xl font-heading font-extrabold text-[var(--app-text)] font-mono">
                ...
                <div className="pt-3 border-t border-[var(--app-border)] space-y-1.5 text-xs">
                  <div className="flex justify-between text-[var(--app-text-muted)]">
                    <span>Target Year: {goal.targetYear}</span>
                    <span className={`font-heading font-semibold ${isHealthy ? 'text-[var(--primary)]' : 'text-[#e59840]'}`}>
                      {goal.status}
                    </span>
                  </div>
                  <div className="flex justify-between text-[var(--app-text-muted)] font-mono text-[11px]">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="bg-[var(--app-surface)] rounded-xl border border-[var(--app-border)] max-w-md w-full p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-[var(--app-border)] pb-3">
              <h4 className="text-base font-heading font-bold text-[var(--app-text)]">Add New Financial Milestone</h4>
            ...
            <form onSubmit={handleCreateGoal} className="space-y-4 text-xs">
              <div>
                <label className="block font-heading font-bold text-[var(--app-text)] mb-1">Milestone Name</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-heading font-bold text-[var(--app-text)] mb-1">Current Saved (₹)</label>
                  <input
                    type="number"
                    min="0"
                    step="10000"
              ...
              <div>
                <label className="block font-heading font-bold text-[var(--app-text)] mb-1">Target Achievement Year</label>
                <input
                  type="number"
                  min="2026"
                  max="2050"
